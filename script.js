document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateIFRFromInput(inputData, depFreq, groundFreq) {
        ...
        if (callsign && departingValue && arrivingValue && routeValue && flightLevelValue) {
            ...
            const fixedRte = routeValue.toUpperCase();

            // Checking for SIDs
            let sid = extractSID(routeText);
            if (sid) {
                routeText = sid;
                let ifr = `ACARS: PDC | CALLSIGN: ${callsign} | EQUIPMENT: ${aircraftValue} | DEPARTURE: ${departingValue} | DESTINATION: ${arrivingValue} | ROUTE: ${fixedRte} | ALTITUDE: ${afterInitial} | SQUAWK: ${transponder} | REMARKS: CLEARED ${routeText} DEPARTURE CLIMB VIA SID EXP ${afterInitial} 10 MIN AFT DP, DPFRQ ${depFreq} CTC ${groundFreq} TO PUSH`;
            ...
            if (routeText === "N/A" || routeText.toLowerCase().includes("gps") || routeText.toLowerCase().includes("gps-direct") || routeText.toLowerCase().includes("dct") || routeText.toLowerCase().includes("direct")) {
                routeText = "DCT";
                let ifr = `ACARS: PDC | CALLSIGN: ${callsign} | EQUIPMENT: ${aircraftValue} | DEPARTURE: ${departingValue} | DESTINATION: ${arrivingValue} | ROUTE: DCT | ALTITUDE: ${afterInitial} | SQUAWK: ${transponder} | REMARKS: CLEARED ${routeText} INITIAL ${initialAltitude} EXP FL${afterInitial} 10 MIN AFT DP, DPFRQ ${depFreq} CTC ${groundFreq} TO PUSH`;
                return ifr.toUpperCase();
            }
